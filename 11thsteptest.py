...
DEFAULT_REPORT_HTML = r"C:\sri\trisulauto\tcp-report-test.html"
...
def _load_fetch_vm_metrics():
    """Dynamically load fetch_vm_metrics from 13thstep.py."""
    step13_path = r"C:\sri\trisulauto\13thstep.py"
    spec = importlib.util.spec_from_file_location("step13_vm_metrics", step13_path)
    if not spec or not spec.loader:
        return None
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return getattr(module, "fetch_vm_metrics", None)
...
def fetch_tcp_analyzer_counters(group_id, output_file=None, infra_snapshot=None):
    ...
    if output_file:
        ...
        fetch_vm_metrics = _load_fetch_vm_metrics()
        if fetch_vm_metrics:
            for ip in ip_list:
                try:
                    ip_vm_data[ip] = fetch_vm_metrics(ip)
                except Exception:
                    ip_vm_data[ip] = {"cpu": 0, "ram": 0, "tcp": 0}
...
""")
                ...
                <tr>
                    <td>{idx}</td>
                    <td>
                        <button class="ip-link" data-ip="{safe_ip}" data-target="{flow_row_id}">{display_ip}</button>
                        <div style="font-size:12px; color:#555; margin-top:5px;">
                            CPU: {cpu}% | RAM: {ram}% | TCP: {tcp}
                        </div>
                    </td>
                </tr>
...
""")
                    ...
                    # Filter to only IPs with meters 0 (Latency Internal), 2 (Retransmitted Packets Internal), and 4 (Retransmission Rate % Internal)
                    required_meters = {0, 2, 4}
                    if required_meters.issubset(ip_meters):
                        html_parts.append("""
                <tr>
                    <td colspan="3">
...
""")
        ...
        # IP-only mode: extract from internal meters 0, 2, and 4 (top 5 each => expected 15 rows).
        selected_ips = []
        required_meters = [0, 2, 4]
        ...
        if selected_ips:
            for ip in selected_ips:
                print(ip)
            print("-" * 50)
            print(f"Total: {len(selected_ips)}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["ip_only", "report", "ip_flows", "serve_api", "report_with_api"], default="ip_only")
    ...
    parser.add_argument("--output-html", default=DEFAULT_REPORT_HTML, help="Output HTML path for report/report_with_api modes")
    ...
    if check_trisul_connection(TRISUL_HOST, TRISUL_PORT):
        try:
            ...
            elif args.mode == "ip_only":
                print("\nExtracting INTERNAL IPs with required meters (0, 2, 4)...\n")
                group_guid = "{E45623ED-744C-4053-1401-84C72EE49D3B}"
                fetch_tcp_analyzer_counters(group_guid, output_file=None, infra_snapshot=infra_snapshot)
            else:
                print("\nFetching TCP Analyzer counters...\n")
                group_guid = "{E45623ED-744C-4053-1401-84C72EE49D3B}"
                html_output = args.output_html
